...
# podkreślanie wyszukiwanej frazy (jak we wzorcu)
...
def build_filters_sql(query, selected_parafia, y_from, y_to, a_from, a_to):
    """
    Buduje fragment WHERE i listę parametrów tak samo,
    jak w /search z pierwszego (wzorcowego) pliku.
    """
    sql = " WHERE 1=1"
    params = []
...
@app.get("/")
def home():
    # Do wyszukiwarki potrzebne są tylko NAZWY parafii
...
def search():
    # Obsługujemy zarówno POST (formularz), jak i GET (link z mapy)
    ...
    # jeśli nic nie podano – zwróć pusty stan (jak we wzorcu)
    if (
        not query
    ...
    with psycopg.connect(DSN, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            where_sql, params = build_filters_sql(
                query, selected_parafia, y_from, y_to, a_from, a_to
            )
...
def export():
    ...
    (query, parafia, rok, wiek). Sortowanie: po id.
    """
